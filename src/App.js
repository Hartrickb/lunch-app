...
import { Button, ChakraProvider, Container, Flex, Grid, GridItem, Heading, Input, Spinner } from '@chakra-ui/react';
...
const App = function () {
  ...
  const letMeChooseSetPlaces = async () => {
    setPlaces([]);
    setWaiting(true);
    const response = await fetch('https://us-central1-lunchapp-330918.cloudfunctions.net/zipCode/letMeChoose', {
      method: 'POST',
      headers: {
        ...
        <ChakraProvider>
          <Container maxWidth="lg" marginBottom="auto" paddingTop="20px">
            <Grid width="full" templateRows="repeat(3, 1fr)" templateColumns="repeat(2, 1fr)" gap={4}>
              ...
              {waiting ? (
                <GridItem colSpan={2}>
                  <Spinner thickness="4px" speed=".7s" emptyColor="white" color="blue.500" size="xl" />
                </GridItem>
              ) : null}
